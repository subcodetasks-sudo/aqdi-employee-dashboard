    ...
    <div className="flex flex-col gap-5">
            ...
            <tbody>
              {TRACKED_EVENTS.map((row) => (
                <tr key={row.event} className={cn(row.highlight && "bg-[#F0FDF4]")}>
                  <td className={cn(TD, "font-mono font-semibold text-[#111827]")}>{row.event}</td>
                  <td className={TD}>{row.meaning}</td>
                  <td className={cn(TD, row.highlight ? "font-bold text-[#15803D]" : "text-[#9CA3AF]")}>
                    {row.value}
                  </td>
      ...
      <SectionCard title="مولّد روابط UTM – لوسم روابط الحملات">
        ...
        <button
          type="button"
          onClick={copyLink}
          className="h-9 px-4 rounded-lg border border-[#E6EBE9] bg-white text-[12px] font-bold text-[#374151] hover:bg-[#F9FAFB] transition-colors flex items-center gap-1.5 w-fit"
        >
        ...
        <div>
          <p className="text-[12px] font-bold text-[#111827] mb-1.5">الرابط الموسوم</p>
          <div className="rounded-lg border border-[#E6EBE9] bg-[#F9FAFB] px-3 py-2.5 text-[12px] text-[#374151] break-all">
            {taggedUrl}
          </div>
        </div>
      </SectionCard>
  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <input
        ...
        className="h-9 rounded-lg border border-[#E6EBE9] px-3 text-[13px] text-[#111827] focus:outline-none focus:border-[#0B5345]"
      />
    </label>
  );
}
...
function IntegrationCard({ item, checked, onToggle }) {
  return (
    <div className="rounded-lg border border-[#E6EBE9] p-3.5 flex flex-col gap-2.5 min-w-0">
      <div className="flex items-start justify-between gap-2">
        <p className="text-[13px] font-bold text-[#111827] min-w-0">{item.name}</p>
        <Switch checked={checked} onCheckedChange={onToggle} className="shrink-0" />
      </div>

      {item.id ? (
        <p className="text-[11px] text-[#9CA3AF]">المعرّف: {item.id}</p>
      ) : (
        <p className="text-[11px] text-[#DC2626] font-semibold">غير مربوط</p>
      )}

      {item.badge ? (
        <span className="text-[10px] font-bold text-[#15803D] bg-[#DCFCE7] rounded px-1.5 py-0.5 w-fit">
          {item.badge}
        </span>
      ) : null}

      {item.note ? <p className="text-[11px] text-[#6B7280]">{item.note}</p> : null}

      {item.events?.length ? (
        <div className="flex flex-wrap gap-1">
          {item.events.map((event) => (
            <span key={event} className="text-[10px] font-mono font-semibold text-[#374151] bg-[#F3F4F6] rounded px-1.5 py-0.5">
              {event}
            </span>
          ))}
        </div>
      ...
      {item.connected ? <p className="text-[11px] text-[#9CA3AF]">{item.lastEvent}</p> : null}

      <button
        type="button"
        onClick={() => toast.success("فتح الإعداد والكود (واجهة تجريبية)")}
        className="h-8 rounded-lg border border-[#E6EBE9] bg-white text-[11px] font-bold text-[#374151] hover:bg-[#F9FAFB] transition-colors mt-auto"
      >
        الإعداد والكود
      </button>
    </div>
