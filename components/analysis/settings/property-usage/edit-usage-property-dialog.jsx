...
export default function EditUsagePropertyDialog({ unit }) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: () =>
      axiosInstance.post(`/admin/real-estate-usages/${unit.id}`, {
        name_ar: unitName,
        contract_type: unitType,
      }),
    onSuccess: (res) => {
      toast.success(res?.data?.message || "تم تعديل استخدام العقار بنجاح");
      setOpen(false);
  ...
  return (
    ...
    >
      <label className="flex flex-col gap-1.5">
        <SettingsFieldLabel required>استخدام العقار</SettingsFieldLabel>
        <Input
