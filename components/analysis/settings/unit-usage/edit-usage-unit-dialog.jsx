...
export default function EditUsageUnitDialog({ unit }) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: () =>
      axiosInstance.post(`/admin/unit-usages/${unit.id}`, {
        name_ar: unitName,
        contract_type: unitType,
      }),
    onSuccess: (res) => {
      toast.success(res?.data?.message || "تم تعديل استخدام الوحدة بنجاح");
      setOpen(false);
      queryClient.invalidateQueries({ queryKey: ["unit-usages"] });
    },
    onError: (error) => {
      toast.error(error?.response?.data?.message || "حدث خطأ أثناء تعديل استخدام الوحدة");
    },
  });
