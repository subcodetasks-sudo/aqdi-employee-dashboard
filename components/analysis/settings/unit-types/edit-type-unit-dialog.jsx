...
export default function EditTypeUnitDialog({ unit }) {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: () =>
      axiosInstance.post(`/admin/unit-types/${unit.id}`, {
        name_ar: unitName,
        contract_type: unitType,
      }),
    onSuccess: (res) => {
      toast.success(res?.data?.message || "تم تعديل نوع الوحدة بنجاح");
      setOpen(false);
      queryClient.invalidateQueries({ queryKey: ["unit-types"] });
    },
    onError: (error) => {
      toast.error(error?.response?.data?.message || "حدث خطأ أثناء تعديل نوع الوحدة");
    },
  });
